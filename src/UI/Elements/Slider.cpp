...
/*   Updated: 2025/09/20 16:43:25 by mbatty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Slider::Slider(UIAnchor anchor, std::string label, glm::vec2 offset, glm::vec2 size, std::function<void(float)> onChange, std::function<void(Slider *)> onUpdate, float startValue)
{
	type = UIElementType::UITYPE_SLIDER;
	_buttonShader = Engine::Shaders->get("gui");
	backgroundTexture = Engine::Textures->get(DIRT_TEXTURE_PATH);
	currentTexture = Engine::Textures->get(STONE_TEXTURE_PATH);
	this->label = label;
	this->offset = offset;
	this->pos = glm::vec2(0);
	this->size = size;
...
void	Slider::update(glm::vec2 mousePos, bool mousePressed)
{
