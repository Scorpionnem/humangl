...
/*   Updated: 2025/10/19 09:49:23 by mbatty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Part::update(const glm::mat4 &parentMat, float deltaTime)
{
	if (!_timeline)
		return ;
	...
	for (Part *part : _children)
		part->update(_mat, deltaTime);
}

void	Part::selectDraw(Shader *shader)
{
	shader->setMat4("model", _mat);
	shader->setVec3("color", _idColor / 255.0f);
